...
double parse_metric(const char *response) {
    ...
    if (json_object_object_get_ex(parsed_json, "ewa_latency", &latency_obj) &&
        ...
        // Compute the product of latency^2 and request_count
        //if(request_count==0){
         //  request_count =1;
       // }
...
// Function to find the server with minimum score in the per-CPU rxcnt1 map
// and store it in the rxcnt map
void find_and_store_min_score_server(int map_fd1, int map_fd) {
    struct map_key key = {0}; // Initialize key to 0
    ...
    // Iterate over the eBPF map to find the minimum score
    while (bpf_map_get_next_key(map_fd1, &key, &key) == 0) {
        if (bpf_map_lookup_elem(map_fd1, &key, percpu_values) == 0) {
            ...
            double average_score = total_score / ncpus;
    ...
    if (min_ip != 0) {
        // Store the minimum score server in the rxcnt map
        key.ip = min_ip;
        value.score = min_score;
        if (bpf_map_update_elem(map_fd, &key.ip, &value.score, BPF_ANY) != 0) {
            fprintf(stderr, "Failed to update rxcnt map with minimum score server: %s\n", strerror(errno));
        } else {
            char ip_str[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &min_ip, ip_str, INET_ADDRSTRLEN);
            printf("\n Stored server with minimum score: %u (score: %f) in rxcnt map\n", key.ip, min_score);
        }
    } else {
        printf("No servers found in the rxcnt1 map or an error occurred.\n");
    }
}
// replica selection method : 

void manage_servers_based_on_score(int map_fd1, int map_fd, double joining_threshold, double leaving_threshold) {
    ...
    double new_average = sum /cnt_servers ;
...
// Shuffle function
void shuffle(char *server_ips[], char *servers[], int n) {
    if (n > 1) {
        struct timeval tv;
        gettimeofday(&tv, NULL);
        unsigned int seed = (unsigned int)(tv.tv_usec ^ pthread_self());

        // Seed the random number generator for this thread
        srand(seed);

        printf("Thread ID: %lu, Seed: %u\n", pthread_self(), seed);

        for (int i = n - 1; i > 0; i--) {
            int j = rand() % (i + 1);

            // Swap in server_ips array
            char *temp_ip = server_ips[j];
            server_ips[j] = server_ips[i];
            server_ips[i] = temp_ip;

            // Swap in servers array
            char *temp_server = servers[j];
            servers[j] = servers[i];
            servers[i] = temp_server;
        }
    }
}
...
void select_random_servers(char *servers[], char *server_ips[], int num_servers, int *index1, int *index2) {
...
void* handle_probing(void* args) {
    ...
    select_random_servers(servers, server_ips, num_servers, &index1, &index2);
 ...
 //  perform_http_request_and_store_server_load(servers[index3], targs->map_fd, targs->map_fd1, server_ips[index3]);
  //  perform_http_request_and_store_server_load(servers[index4], targs->map_fd, targs->map_fd1, server_ips[index4]);
    ...
    manage_servers_based_on_score(targs->map_fd1, targs->map_fd, joining_threshold, leaving_threshold);
